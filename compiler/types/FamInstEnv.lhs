...
{-
            case tyConFamilyCoercion_maybe (fi_tycon famInst) of
              Just ax -> ppr ax
...
pprFamInstHdr (FamInst {fi_axiom = axiom, fi_flavor = flavor})
  ...
  where
    Just (fam_tc, tys) = coAxiomSplit_maybe axiom 
...
mkLocalFamInst (ACoAxiom axiom)
  = case coAxiomSplit_maybe axiom of
      Nothing           -> panic "FamInstEnv.mkLocalFamInst 1"
      Just (tycon, tys) -> ASSERT2 ( isSynTyCon tycon, ppr tycon )
        FamInst {
...
mkImportedFamInst fam mb_tcs flavor axiom
  = FamInst {
      ...
      fi_tcs    = mb_tcs,
      fi_tvs    = mkVarSet . coAxiomTyVars $ axiom,
      fi_tys    = case coAxiomSplit_maybe axiom of
...
overwriteFamInstEnv inst_env ins_item@(FamInst {fi_fam = cls_nm, fi_tcs = mb_tcs})
  ...
  where
    ...
    (fam, tys) = expectJust "FamInstEnv.overwriteFamInstEnv"
               $ coAxiomSplit_maybe inst_axiom
