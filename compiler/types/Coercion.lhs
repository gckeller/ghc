...
module Coercion (
        ...
        coAxiomTypeFamTyCon, coAxiomDataFamTyCon, coAxiomSplit_maybe,
...
coAxiomTypeFamTyCon ax = case tyConAppTyCon_maybe (coAxiomLHS ax) of
                           Nothing -> Nothing
                           Just tc -> if isSynFamilyTyCon tc
                                      then Just tc
                                      else Nothing
...
coAxiomDataFamTyCon ax = case tyConAppTyCon_maybe (coAxiomLHS ax) of
                           Nothing -> Nothing
                           Just tc -> if isDataFamilyTyCon tc
                                      then tyConAppTyCon_maybe (coAxiomRHS ax)
                                      else Nothing

-- If `ax :: F a ~ b`, and `F` is a family instance, returns (F, [a])
coAxiomSplit_maybe :: CoAxiom -> Maybe (TyCon, [Type])
coAxiomSplit_maybe ax
  = case splitTyConApp_maybe (coAxiomLHS ax) of
      r@(Just (tc,_)) -> if isFamilyTyCon tc
                         then r
                         else pprTrace "not isFamilyTyCon" (ppr tc) $ Nothing
      Nothing         -> Nothing
\end{code}
