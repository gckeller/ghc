...
buildAlgTyCon tc_name ktvs stupid_theta rhs is_rec gadt_syn
  ...
  = -- We need to tie a knot as the coercion of a data instance depends
    ...
    fixM $ \ tycon_rec -> do 
    { fam_parent <- mkFamInstParentInfo tc_name ktvs fam_inst_info (Left tycon_rec)
...
mkFamInstParentInfo tc_name tvs (family, instTys) rep
  = do { -- Create the coercion
       ; co_tycon_name <- newImplicitBinder tc_name mkInstTyCoOcc
       ; let co_tycon = case rep of
                          Left rep_tycon  -> mkDataFamInstCo co_tycon_name tvs
                                               family instTys rep_tycon
                          Right inst_type -> mkTypeFamInstCo co_tycon_name tvs
                                               family instTys inst_type
